...
class MaskedAutoencoderViT(nn.Module):
    ...
    def __init__(
        ...
        self.patch_embed = patch_embed(
            ...
            num_frames,
            t_patch_size,
        ) if mask_UM_flag is None else patch_embed(
            int(img_size/2),
            patch_size,
            in_chans,
            embed_dim,
        ...
        self.UM_mask_token = nn.Parameter(torch.zeros(1, num_frames, 1))
        ...
        # 上采样
        # self.mask_um_up = nn.Sequential(
        #     nn.Conv2d(num_frames, num_frames, kernel_size=3, padding=1, bias=False),
        #     nn.BatchNorm2d(num_frames),
        #     nn.ReLU(inplace=True),
        # )

        self.mask_um_up = nn.Sequential(
            nn.Conv2d(num_frames, num_frames * 2, kernel_size=3, padding=1, bias=False),
            nn.BatchNorm2d(num_frames * 2),
            nn.ReLU(inplace=True),
            nn.Conv2d(num_frames * 2, num_frames, kernel_size=3, padding=1, bias=False),
            nn.BatchNorm2d(num_frames),
        )
        ...
        self.my_avg_pool = nn.AvgPool2d(kernel_size=8, stride=8)
    ...
    def unpatchify_up(self, x):
        ...
        x = x.reshape(shape=(N, t, int(h/2), int(w/2), u, p, p, 1))

        x = torch.einsum("nthwupqc->nctuhpwq", x)
        imgs = x.reshape(shape=(N, T, int(H/2), int(W/2)))
        return imgs, (N, T, H, W)
    ...
    # 逐pixelmask
    def forward_encoder(self, x, mask_ratio, mask_UM_origion):
        
        if mask_UM_origion is not None:
            #将x最后两个维度合并
            #x.shape torch.Size([1, 72, 128, 128])
            x = x.reshape(x.shape[0], x.shape[1], -1)
            mask_UM = torch.unsqueeze(mask_UM_origion, dim=1)
            mask_UM = mask_UM.repeat(1, x.shape[1], 1)
            x = x[~mask_UM].reshape(x.shape[0], x.shape[1], -1)
            temp_h = temp_w = int(np.sqrt(x.shape[2]))
            x = x.reshape(x.shape[0], x.shape[1], temp_h, temp_w)
        # mask_UM_flag 为None时，使用逐patchmask，否则使用逐pixelmask
    ...
    # 逐patchmask
    def forward_encoder_patch_wise_mask(self, x, mask_ratio, mask_UM):
        # 维度转换
        # x = x[:, :-1, :, :]  # 切片处理数据维度
        x = torch.unsqueeze(x, dim=1)
        # embed patches
        ...
        if mask_UM is not None:
            mask_UM = torch.unsqueeze(mask_UM, dim=1)
            mask_UM = mask_UM.repeat(1, T, 1)
            x = x[~mask_UM].reshape(N, -1, C)
            # mask_ratio = round(float(1-(1-mask_ratio)/(1-0.75)),2) # (1-0.75) UM * (1-0.6) RM = (1-0.9) mask
        else:
            x = x.reshape(N, T * L, C)
    ...
    # 逐pixelmask
    def forward_decoder(self, x, ids_restore, mask_UM_origion):
        N = x.shape[0]
        ...
        # 上采样
        if mask_UM_origion is not None:
            pred_up, shape_up = self.unpatchify_up(x)
            pred_up = pred_up.reshape(pred_up.shape[0], pred_up.shape[1], -1)
            mask_UM = torch.unsqueeze(mask_UM_origion, dim=1)
            mask_UM = mask_UM.repeat(1, pred_up.shape[1], 1)
            UM_mask_token = self.UM_mask_token.repeat(shape_up[0], 1, shape_up[2]*shape_up[3]-pred_up.shape[2])
            pred_up_ = torch.zeros((shape_up[0], shape_up[1], shape_up[2]*shape_up[3]), dtype=UM_mask_token.dtype, device=UM_mask_token.device)
            # pred_up_.masked_scatter(mask_UM, UM_mask_token)
            pred_up_[mask_UM] = UM_mask_token.view(-1)
            pred_up_[~mask_UM] = pred_up.to(UM_mask_token.dtype).view(-1)
            # pred_up_.masked_scatter(~mask_UM, pred_up.to(UM_mask_token.dtype))
            temp_h = temp_w = int(np.sqrt(pred_up_.shape[2]))
            pred_up_ = pred_up_.reshape(pred_up_.shape[0], pred_up_.shape[1], temp_h, temp_w)
            # cpu
            # pred_up_ = self.mask_um_up.cpu()(pred_up_.cpu().to(UM_mask_token.dtype))    
            pred_up_ = self.mask_um_up(pred_up_.to(UM_mask_token.dtype))
    ...
    # def spectral_angle_loss(self, x, y):
    #     # 确保输入为一维（向量）
    #     x = x.view(-1)
    #     y = y.view(-1)
    ...
    def forward_loss(self, imgs, pred, mask, mask_UM_origion, pred_up):
        """
        imgs: [N, 3, T, H, W]
        ...
        pred_up = torch.unsqueeze(pred_up, dim=1)
        ...
        if mask_UM_origion is not None:
            pred_whole = self.patchify(pred_up).reshape(N, t, h * w, 192)
        else:
        ...
        if mask_UM_origion is not None:
            loss1 = (self.patchify(pred_up) - target1) ** 2
        else:
        ...
        # loss2 = self.spectral_angle_loss(target_spectral,pred_spectral)

        # 约束每个8*8区域光谱
        if mask_UM_origion is not None:
            pred_spectral = self.my_avg_pool(torch.squeeze(pred_up, dim=1))
        else:
            pred_spectral = self.my_avg_pool(torch.squeeze(self.unpatchify(pred), dim=1))
        target_spectral = self.my_avg_pool(torch.squeeze(_imgs, dim=1))
        loss2 = self.spectral_angle_loss(target_spectral, pred_spectral)
        ...
        loss = loss1 + 0.01* loss2 + loss3  # mean loss on removed patches
        return loss, [loss1,0.01*loss2,loss3]

    def forward(self, imgs, mask_ratio=0.9, mask_UM=None):
        latent, mask, ids_restore = self.forward_encoder(imgs, mask_ratio, mask_UM)
        _ = self.patchify( torch.unsqueeze(imgs, dim=1))
        pred, pred_up = self.forward_decoder(latent, ids_restore, mask_UM)  # [N, L, p*p*3]
        loss, loss_list = self.forward_loss(imgs, pred, mask, mask_UM, pred_up)
        
        # return loss, pred, mask
        if mask_UM is not None:
            return loss, imgs, torch.unsqueeze(pred_up, dim=1), loss_list
        else:
            return loss, imgs, self.unpatchify(pred), loss_list

class MaskUpsampleRefine(nn.Module):
    def __init__(self, num_frames):
        super().__init__()
        self.refine = nn.Sequential(
            nn.Conv2d(num_frames, num_frames, kernel_size=3, padding=1),
            nn.ReLU(inplace=True),
            nn.Conv2d(num_frames, num_frames, kernel_size=3, padding=1),
        )

    def forward(self, x):
        # 自动对齐模块 dtype 与输入 dtype（避免 float16 报错）
        if x.dtype != self.refine[0].weight.dtype:
            self.refine = self.refine.to(dtype=x.dtype)

        # 自动关闭 cuDNN（解决 float16 崩溃问题）
        with torch.backends.cudnn.flags(enabled=False):
            with torch.cuda.amp.autocast(enabled=x.dtype == torch.float16):
