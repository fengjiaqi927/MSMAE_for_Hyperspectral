...
def create_model(nb_classes, weight_path, num_frames, img_size, pretrain=False):
    ...
    if pretrain:
        ...
        for k in ['pos_embed', 'patch_embed.proj.weight', 'patch_embed.proj.bias', 'head.weight', 'head.bias','pos_embed_spatial','pos_embed_temporal']:
            if k in checkpoint_model and checkpoint_model[k].shape != state_dict[k].shape:
                print(f"Removing key {k} from pretrained checkpoint")
                del checkpoint_model[k]
...
def main(args):
    ...
    if args.test_only:
        ...
        confmat = evaluate_without_bg(model, test_data_loader, device=device, num_classes=num_classes, vis_path=args.output_dir + 'vis/', epoch=0)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # gpu几这里填几
        # confmat = evaluate(model, test_data_loader, device=device, num_classes=num_classes)
        confmat = evaluate_without_bg(model, test_data_loader, device=device, num_classes=num_classes, vis_path=args.output_dir + 'vis/', epoch=epoch)
        val_info = str(confmat)
        print(val_info)
...
if __name__ == "__main__":
    ...
    if (args.output_dir + 'vis/'):
        mkdir(args.output_dir+ 'vis/')
